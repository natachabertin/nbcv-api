...
def get_all(db: Session, skip: int = 0, limit: int = 100) -> List[Training]:
    return list_items(db, mTraining, skip, limit)


def create(db: Session, training: TrainingCreate) -> mTraining:
    return create_item(db, training, mTraining)


def update(
        db: Session, training_id: int, training_submit: TrainingUpdate
        ) -> mTraining:
