...
@router.get('/projects', name='all_projects', response_model=List[Project])
def get_projects(
...
@router.post('/projects', name='add_project', status_code=201, response_model=Project)
...
@router.put('/projects/{project_id}', name='update_project')
def update_project(project_id: int, db: Session = Depends(get_db)) -> Project:
    return Project


@router.delete('/projects/{project_id}', name='delete_project')
