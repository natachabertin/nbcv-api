...
@router.get('/trainings', name='all_trainings', response_model=List[Training])
def get_trainings(
...
@router.post('/trainings', name='add_training', status_code=201, response_model=Training)
def create_training(training: Training, db: Session = Depends(get_db)) -> Training:

    return crud.create(db=db, training=training)


@router.get('/trainings/{training_id}', name='get_training')
...
@router.put('/trainings/{training_id}', name='update_training')
def update_training(training_id: int, db: Session = Depends(get_db)) -> Training:
    return Training


@router.delete('/trainings/{training_id}', name='delete_training')
